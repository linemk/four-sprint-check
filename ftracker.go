...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	clrs := (runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * (weight / mInKm) * (duration * minInH)
	return clrs
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	var averageSpeedInSec float64 = math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)
	var clrs float64 = ((walkingCaloriesWeightMultiplier*weight + (averageSpeedInSec/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return clrs
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	var clrs float64 = (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return clrs
}
